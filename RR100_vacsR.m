...
tend = 100; % 100; % 105 for RR during the delayed tau days, 100 for the first 100 days
eend = tend/hh;
     for ee1 = 1:e1
         epsilon1 = EPS1(ee1);
         for ee2 = 1:e2
            ...
            for ii = 1:m
                 ...
                 RdeathP(ee1, ee2, ii) = Dp(eend)-Dp(Rtaui);
                 RdeathR(ee1, ee2, ii) = Ds(eend)-Ds(Rtaui);
                 RhospP(ee1, ee2, ii) = sum(Phosp(Rtaui:eend))*hh;
                 RhospR(ee1, ee2, ii) = sum(Shosp(Rtaui:eend))*hh;
                 RinciP(ee1, ee2, ii) = sum(Pinci(Rtaui:eend))*hh;
                 RinciR(ee1, ee2, ii) = sum(Sinci(Rtaui:eend))*hh;
             end
         end
     end
