     ...
     for ee1 = 1:e1
         ...
         for ee2 = 1:e2
            ...
            for ii = 1:m
                 ...
                 Dtaui = Dtau/hh;
                 [Pinci, Sinci, inciS, inciV1, inciV1w, inciE1, inciE1w, inciV2, inciV2w, inciW2, inciW2w, inciR1w, inciR2w, Dp, Ds, Phosp, Shosp, nu1, nu2, nu2w] = vacsD(hh, Dtau, DV, epsilon1, epsilon2, tend);
              
                 DdeathP(ee1, ee2, ii) = Dp(eend)-Dp(Rtaui); % total Primary infection during the delayed tau days
                 DdeathR(ee1, ee2, ii) = Ds(eend)-Ds(Rtaui); % total Re-infection during the delayed tau days
                 DhospP(ee1, ee2, ii) = sum(Phosp(Rtaui:eend))*hh; % total hosp from Primary infection
                 DhospR(ee1, ee2, ii) = sum(Shosp(Rtaui:eend))*hh; % total hosp from Re-infection
                 DinciP(ee1, ee2, ii) = sum(Pinci(Rtaui:eend))*hh; % total death from Primary infection
                 DinciR(ee1, ee2, ii) = sum(Sinci(Rtaui:eend))*hh; % total death from Re-infection
             end
         end
     end

save('RR100_D1400.mat', "DinciR","DinciP", "DhospR","DhospP","DdeathR","DdeathP")
%%
% Simulation for optimal dosing interval with waning parameter set I/II

function [Pinci, Sinci, inciS, inciV1, inciV1w, inciE1, inciE1w, inciV2, inciV2w, inciW2, inciW2w, inciR1w, inciR2w, Dp, Ds, Phosp, Shosp, nu1, nu2, nu2w] = vacsD(hh, Dtau, DV, epsilon1, epsilon2, tend)
