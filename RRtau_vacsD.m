...
hh = 0.05;
TAUEND = 15; 
Rtaui = 21/hh; % recommended dosing interval
...
DdeathP = zeros(e1, e2, m);
DdeathR = zeros(e1, e2, m);
DhospP = zeros(e1, e2, m);
DhospR = zeros(e1, e2, m);
DinciP = zeros(e1, e2, m);
DinciR = zeros(e1, e2, m);


DV = 400; % 400 % maxinum number of doses everyday, D
%tend = 100; % 100 for the first 100 days
%eend = tend/hh;
     for ee1 = 1:e1
         epsilon1 = EPS1(ee1);
         for ee2 = 1:e2
             epsilon2 = EPS2(ee2);            
            for ii = 1:m
                 Dtau = TAU(ii); % delayed dosing interval
                 Dtaui = Dtau/hh;
                 [Pinci, Sinci, inciS, inciV1, inciV1w, inciE1, inciE1w, inciV2, inciV2w, inciW2, inciW2w, inciR1w, inciR2w, Dp, Ds, Phosp, Shosp, nu1, nu2, nu2w] = vacsD(hh, Dtau, DV, epsilon1, epsilon2);
              
                 DdeathP(ee1, ee2, ii) = Dp(Dtaui)-Dp(Rtaui); % total Primary infection during the delayed tau days
                 DdeathR(ee1, ee2, ii) = Ds(Dtaui)-Ds(Rtaui); % total Re-infection during the delayed tau days
                 DhospP(ee1, ee2, ii) = sum(Phosp(Rtaui:Dtaui))*hh; % total hosp from Primary infection
                 DhospR(ee1, ee2, ii) = sum(Shosp(Rtaui:Dtaui))*hh; % total hosp from Re-infection
                 DinciP(ee1, ee2, ii) = sum(Pinci(Rtaui:Dtaui))*hh; % total death from Primary infection
                 DinciR(ee1, ee2, ii) = sum(Sinci(Rtaui:Dtaui))*hh; % total death from Re-infection
             end
         end
     end

save('D1_400.mat', "DinciR","DinciP", "DhospR","DhospP","DdeathR","DdeathP")
%%
% Simulation for optimal dosing interval with waning parameter set I/II

function [Pinci, Sinci, inciS, inciV1, inciV1w, inciE1, inciE1w, inciV2, inciV2w, inciW2, inciW2w, inciR1w, inciR2w, Dp, Ds, Phosp, Shosp, nu1, nu2, nu2w] = vacsD(hh, Dtau, DV, epsilon1, epsilon2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if i > Dtaui
        vwttauh = zeros(1, Dtaui+1);
     for uu = 1:Dtaui
          intLambda1(i-Dtaui) = 0;
          intLambda2(i-uu) = 0;
        for ss = (i-Dtaui):(i)
            intLambda1(ss+1) = intLambda1(ss) + Lambda(ss)*hh;
        end
        vttau(i) = nu1(i-Dtaui)*exp(-xi1*Dtau - beta*(1-epsilon1)*intLambda1(i+1));
        for ss2 = (i-uu):i
            intLambda2(ss2+1) = intLambda2(ss2) + Lambda(ss2)*hh;
        end
        vwttauh(uu+1) = vwttauh(uu) +xi1*nu1(i-Dtaui)*exp(-xi1*(Dtaui-uu)*hh)*exp(-beta*(1-epsilon1)*(intLambda1(i)-intLambda2(i-uu+1)))*exp(-beta*intLambda2(i-uu+1))*hh;
     end 
     vwttau(i) = vwttauh(Dtaui+1);
    end
    ...
    if i < Dtaui + hh % in the first 0 - tau days, no one receive the 2nd dose
        nu1(i) = DV;
        nu2(i) = 0;
        nu2w(i) = 0;
    end
